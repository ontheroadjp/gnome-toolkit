...
- `scripts/voice-input/` — whisper.cpp を使ったオフライン音声入力。`Ctrl+Shift+=`
  ...
  `install.sh` で whisper.cpp のビルド・モデルダウンロード・GNOME ショートカット登録を行う。
- `scripts/tmux-switch-us-input/` — tmux の pane 切り替え時に GNOME 入力ソースを
  US キーボードへ自動切替するスクリプト。`focus-us-input@local` GNOME 拡張の
  D-Bus メソッドを経由して切り替える。`install.sh` で
  `~/.local/bin/switch-input-to-us` へのシンボリックリンクを作成する。
